  ...
  return (
    <section className="bg-white" aria-labelledby={`${section.id}-heading`}>
      <div className="mx-auto grid max-w-7xl items-center gap-10 px-4 py-16 sm:px-6 lg:grid-cols-2 lg:px-8 lg:py-20">
        <div className={`min-w-0 overflow-hidden rounded-2xl ${section.imageFirst ? "lg:order-1" : "lg:order-2"}`}>
          <Image
            src={section.image}
        ...
        <div className={`flex min-w-0 flex-col gap-4 ${section.imageFirst ? "lg:order-2" : "lg:order-1"}`}>
          <h2 id={`${section.id}-heading`} className="text-3xl font-bold text-ink sm:text-4xl">
            {section.title}
          </h2>
